...
| 问题 | 处理 |
|------|------|
| 页面无组合 | 确认 API `RUN_SEED=true`；或 `make seed` |
| 净值曲线异常 | 使用「重置 NAV」或删除异常 `nav_snapshots` 后 backfill |
| AkShare 超时 | 设 `DATA_PROVIDER=mock` 演示；生产重试或缩小同步范围 |
| Agent 总是规则引擎 | 检查 `AGENT_MODE=llm` 与 `OPENAI_API_KEY`；Settings 健康检查 |
| 批准决策失败「证据不足」 | 补充 `references` 或关闭 `REQUIRE_LOW_EVIDENCE_BLOCK=false` |
| Phase 1 未完成 | `GET /api/v1/onboarding/status` 或 `make onboarding-check` |
| 迁移 / 缺列 `memory_tier` | 默认 `ALEMBIC_UPGRADE_ON_START=true`；或重启 API（启动时会 `schema_compat` 补列）；手动：`docker compose -f docker-compose.prod.yml exec api alembic upgrade head` |
