...
| 方式 | 能力 |
|------|------|
| **静态预览** `preview/aims-preview.html` | 仅布局示意，**无 API、无真实数据** |
| **完整产品** `docker compose -f docker-compose.prod.yml up` | 组合、决策、研究、同步、Agent 全功能 → http://localhost:8080 |

```bash
cd preview && python3 -m http.server 8080
# http://localhost:8080/aims-preview.html — 页面顶部有「仅布局预览」提示
```

GitHub Pages（静态）：https://chenluwu2026.github.io/copilot-test/

**基金经理一日**操作顺序：[`docs/DAILY_OPERATOR_PLAYBOOK.md`](./docs/DAILY_OPERATOR_PLAYBOOK.md)。Phase 1 验收：`GET /api/v1/onboarding/status`。
